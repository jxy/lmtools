...
### Changed
- Refactored `argo.go` into `argo/cmd/main.go` calling into extracted packages.
  Core logic unchanged; behavior preserved.

### CI/Tooling
- Added `golangci-lint` config (enabling `gofumpt`).
- CI runs `go fmt`, `go test`, `golangci-lint run`, `go vet` on `argo` packages.
  Root-level files (other mains) are not linted to avoid conflicts.

### Formatting
- Applied `gofmt` via `go fmt ./argo` in CI; additional stricter formatting available via `gofumpt`.

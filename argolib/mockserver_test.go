...
func TestMockServer_BasicChat(t *testing.T) {
	...
	// Make a chat request
	req, _, err := BuildRequest(cfg, "Test message")
	if err != nil {
...
func TestMockServer_Embedding(t *testing.T) {
	...
	req, _, err := BuildRequest(cfg, "Test embedding")
	if err != nil {
		t.Fatalf("Failed to build request: %v", err)
	}
...
func TestMockServer_StreamChat(t *testing.T) {
	...
	req, _, err := BuildRequest(cfg, "Test streaming")
	if err != nil {
		t.Fatalf("Failed to build request: %v", err)
	}
...
func TestMockServer_CustomResponse(t *testing.T) {
	mock := NewMockServer(
		WithResponseFunc(func(r *http.Request) (interface{}, int, error) {
			...
			if len(req.Messages) > 0 {
				...
				response = "You said: " + lastMsg.Content
			}
	...
	req, _, err := BuildRequest(cfg, "Hello custom")
	if err != nil {
		t.Fatalf("Failed to build request: %v", err)
	}
...
func TestMockServer_ErrorSimulation(t *testing.T) {
	...
	req, _, err := BuildRequest(cfg, "This should fail")
	if err != nil {
		t.Fatalf("Failed to build request: %v", err)
	}
...
func TestMockServer_RequestCapture(t *testing.T) {
	...
	for _, msg := range messages {
		req, _, err := BuildRequest(cfg, msg)
		if err != nil {
			t.Fatalf("Failed to build request: %v", err)
		}

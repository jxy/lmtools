...
func TestLazyFileConcurrentCreation(t *testing.T) {
	...
	for i := 0; i < numGoroutines; i++ {
		...
		go func(id int) {
			defer wg.Done()
			GetLogger().Debugf("Concurrent write from goroutine %d", id)
		}(i)
	}
...
func TestLazyFileHandlesFailure(t *testing.T) {
	...
	GetLogger().Infof("Testing failure handling")
	...
	_ = os.MkdirAll(logSubDir, constants.DirPerm)

	// Try to write again - with simplified implementation, it will retry
	GetLogger().Infof("Testing after directory recreated")
...
func TestLazyFileNoCreationForReadOnly(t *testing.T) {
	...
	GetLogger().Debugf("This should not create a file")
	GetLogger().Infof("Neither should this")
...
func TestLazyFileCloseSemantics(t *testing.T) {
	...
	GetLogger().Infof("Before close")

	// Close the logger
	Close()

	// Try to write after close - should not create new file
	GetLogger().Infof("After close - should not appear")

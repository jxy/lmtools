...
func buildTestRequestWithMessages(t *testing.T, cfg TestRequestConfig) RequestBuild {
	t.Helper()
	req, body, err := BuildChatRequest(cfg, mockToolInteractionMessages(), ChatBuildOptions{Stream: cfg.StreamChat})
	if err != nil {
...
func mockToolInteractionMessages() []TypedMessage {
	return []TypedMessage{
...
func TestBuildChatRequestWithSessionMessages_Providers(t *testing.T) {
	tests := []struct {
		name           string
		provider       string
		model          string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rb := buildTestRequestWithMessages(t, cfg)

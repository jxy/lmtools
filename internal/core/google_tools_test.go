...
func TestBuildGoogleToolAwareRequest(t *testing.T) {
	...
	req, body, err := buildGoogleTestRequest(cfg, messages, "gemini-1.5-pro", toolDefs, nil, false)
	if err != nil {
...
func TestGoogleToolExecutionFlow(t *testing.T) {
	...
	// Build request
	_, body, err := buildGoogleTestRequest(cfg, messages, "gemini-1.5-pro", toolDefs, nil, false)
	if err != nil {
	...
	_, body2, err := buildGoogleTestRequest(cfg, messagesWithTool, "gemini-1.5-pro", toolDefs, nil, false)
	if err != nil {

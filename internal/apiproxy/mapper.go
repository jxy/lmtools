...
func (m *ModelMapper) MapModel(model string) (provider, mappedModel string) {
	...
	switch m.config.PreferredProvider {
	case "google":
...
func (m *ModelMapper) mapToSmallModel() (provider, model string) {
	...
	if m.config.PreferredProvider == "google" && m.isGeminiModel(smallModel) && m.config.GeminiAPIKey != "" {
		return "gemini", smallModel
	}
	if m.config.PreferredProvider == "argo" && m.isArgoModel(smallModel) && m.config.ArgoUser != "" {
...
func (m *ModelMapper) mapToBigModel() (provider, model string) {
	...
	// Determine provider based on model and API key availability
	if m.config.PreferredProvider == "google" && m.isGeminiModel(bigModel) && m.config.GeminiAPIKey != "" {
		return "gemini", bigModel
	}
	if m.config.PreferredProvider == "argo" && m.isArgoModel(bigModel) && m.config.ArgoUser != "" {
...
func GetProviderURL(provider string) string {
	...
	case "argo":
		// This will be determined by the Argo environment configuration
		return ""
	default:
		return ""
	}
...
// GetArgoURL returns the appropriate Argo URL based on environment
func GetArgoURL(env string, endpoint string) string {
	base := ""
	switch env {
	case "prod":
		base = "https://apps.inside.anl.gov/argoapi/api/v1/resource"
	case "dev", "":
		base = "https://apps-dev.inside.anl.gov/argoapi/api/v1/resource"
	default:
		// Custom URL
		base = env
	}

...
func TestConvertAnthropicToOpenAI(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expected: &OpenAIRequest{
				Model:     "gpt-4",
				MaxTokens: 20000, // No longer capped, pass through as-is
				Messages: []OpenAIMessage{
					{
						Role:    RoleUser,
						Content: "Hello",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := converter.ConvertAnthropicToOpenAI(tt.input)
...
func TestConvertAnthropicToArgo(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := converter.ConvertAnthropicToArgo(tt.input, tt.user)
			if (err != nil) != tt.wantErr {

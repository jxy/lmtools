...
func TestLoggingIntegration(t *testing.T) {
	...
	// Create mock server
	ms := argo.NewMockServer()
	defer ms.Close()

	// Set up log directory
	logDir := filepath.Join(tmpHome, ".argo", "logs")
	...
	for _, entry := range entries {
		...
		if err == nil && now.Sub(info.ModTime()) < time.Minute {
			...
			if strings.Contains(name, "_argo_") && strings.HasSuffix(name, ".log") {
				processLogFound = true
			}
			if strings.Contains(name, "_embed_input_") && strings.HasSuffix(name, ".json") {
	...
	for _, entry := range entries {
		if strings.Contains(entry.Name(), "_chat_input_") && strings.HasSuffix(entry.Name(), ".json") {
			info, err := entry.Info()
			if err == nil && now.Sub(info.ModTime()) < time.Minute {
